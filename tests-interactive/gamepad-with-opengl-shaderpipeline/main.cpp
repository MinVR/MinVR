...
#include <MinVR-2.0/MinVR_Gainput/VRGainputDevice.h>
#include <MinVR-2.0/input/VRInputDevice.h>
...
public:
  ...
  bool isVibrating = false;
    ...
    MyVRApp(int argc, char** argv) : VRApp(argc, argv) {
      ...
      std::vector<VRInputDevice*> inputDevices = getInputDevices();

      for (int i = 0; i < inputDevices.size();i++)
      {
        VRGainputDevice* vrdevice = dynamic_cast<VRGainputDevice*>(inputDevices[i]);
        if (vrdevice != nullptr)
        {
          vrGainPutDevice = vrdevice;
        }
      }
    ...
    void onButtonDown(const VRButtonEvent &state) {
        ...
        if (state.getName().find("pad_button_a") != std::string::npos)
        {
         
          gainput::InputDevicePad* pad = static_cast<gainput::InputDevicePad*>(vrGainPutDevice->getGainInputManager()->GetDevice(0));
          !isVibrating ? pad->Vibrate(1.0f, 0.0f) : pad->Vibrate(0.0f, 0.0f);
