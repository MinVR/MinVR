...
public:
  ...
  // VRMain is a singleton class -- each program should contain exacly one instance of VRMain.  Access the
  // current instance using the instance() function, which will create a new instance the first time it is
  // called, typically via:  VRMain::instance()->initialize(settings);
  ...
  // BEGIN LOOP: YOUR PROGRAM SHOULD CONTAIN A MAIN LOOP THAT CALLS THE FOLLOWING TWO FUNCTIONS ONCE PER FRAME:
    ...
    virtual void renderOnAllDisplayDevices(void (*renderCallbackFunction)(const VRRenderState &));
